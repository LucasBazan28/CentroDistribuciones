...
export async function fetchProductById(id: number): Promise<any> {
  try {
    ...
    const { data, error } = await supabase
      .from("articulos")
      .select(
        "id, referencia, descripcion, precio_unitario, precio_venta, moneda_id, stock, categoria_id, marca_id, grupo_descuento_id, imageURL, iva, observacion, marcas(nombre), grupo_descuento(nombre, descuento), categorias(nombre), unidad_medida_id, unidad_medida(nombre)"
      )
      .eq("id", id)
      .eq("activo", true)
      .maybeSingle()
